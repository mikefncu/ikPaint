...
void kpColorCellsBase::setColor( int column, const QColor &colorIn, const QString & name)
{
    const int tableRow = column % rowCount(); // / columnCount();
    const int tableColumn = column / rowCount(); // % columnCount();
        ...
        bool allowEmptyCell) const
{
    ...
    const int cell = c * rowCount() + r; //  r * columnCount() + c;
        ...
        if(e->x() > d->mousePos.x()+delay || e->x() < d->mousePos.x()-delay ||
           e->y() > d->mousePos.y()+delay || e->y() < d->mousePos.y()-delay){
            ...
            if (cell != -1)
            {
               ...
               QPixmap tempPixmap = QPixmap(20, 20);
               QPainter painter;
               painter.begin(&tempPixmap);
               painter.fillRect(tempPixmap.rect(), d->colors[cell]);
               painter.end();

               mimeData->setColorData(d->colors[cell]);
               drag->setMimeData(mimeData);
               drag->setPixmap(tempPixmap);
               drag->setHotSpot(QPoint(tempPixmap.width()/2, tempPixmap.height()));
...
void kpColorCellsBase::dragEnterEvent( QDragEnterEvent *event)
{
     ...
     kDebug () << "kpColorCellsBase::dragEnterEvent() acceptDrags="
               ...
               << endl;
#endif
     event->setAccepted( d->acceptDrags && event->mimeData ()->hasColor());
     if (event->isAccepted ())
...
void kpColorCellsBase::dropEvent( QDropEvent *event)
{
     ...
     if( c.isValid()) {
          ...
          setColor(cell, c, c.name(QColor::HexArgb).toUpper());
          #else
          setColor(cell, c, c.name());
          #endif
          ...
          if (event->dropAction () == Qt::MoveAction && dragSourceCell != -1) {
              setColor(dragSourceCell, destOldColor, destOldColor.name(QColor::HexArgb).toUpper());
          }
     }
}
...
void kpColorCellsBase::mouseReleaseEvent( QMouseEvent *e )
{
	...
	if ( (cell != -1) && (d->selected != cell) )
	{
        ...
        const int newRow = cell % rowCount();
